...
def load_multiomics_benchmark_ds(dataset, preprocessing, dir_path=None):
    ...
    assert preprocessing in [
        "Aligned",
        "Original",
    ], f"Preprocessing {preprocessing} not in ['Aligned', 'Original']"
    ...
    path_to_benchmark_ds_pre = os.path.join(
        dir_path,
    ...
    cnv_data = pd.read_csv(
        os.path.join(path_to_benchmark_ds_pre, f"{dataset}_CNV{file_appendix}.csv"), index_col=0
    )
    ...
    methylation_data = pd.read_csv(
        os.path.join(path_to_benchmark_ds_pre, f"{dataset}_Methy{file_appendix}.csv"), index_col=0
    )
    ...
    rna_data = pd.read_csv(
        os.path.join(path_to_benchmark_ds_pre, f"{dataset}_mRNA{file_appendix}.csv"), index_col=0
    )
    ...
    mirna_data = pd.read_csv(
        os.path.join(path_to_benchmark_ds_pre, f"{dataset}_miRNA{file_appendix}.csv"), index_col=0
    )
...
def load_multiomics_benchmark_shamir(dataset, normalize, aligned=True, subtype_labels=False):
    assert (
        dataset in ALL_MULTIOMICS_DATASETS_SHAMIR
    ), f"Dataset {dataset} not in {ALL_MULTIOMICS_DATASETS_SHAMIR}"
    shamir_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "..",
        "benchmark_data",
        "shamir_data",
    ...
    methylation_data = pd.read_table(
        os.path.join(dir_path, "methy"), header=0, index_col=0, sep=r"\s+"
    )
    methylation_data = methylation_data.T
    methylation_data = methylation_data[~methylation_data.index.duplicated(keep="first")]
    illumina_mapping = pd.read_csv(
        os.path.join(shamir_path, "methylation_gene_mapping.csv"),
        skiprows=7,
        index_col=0,
        dtype=str,
    )
    ...
    if subtype_labels and dataset in ["gbm", "breast", "colon", "kidney", "sarcoma"]:
        if dataset == "colon":
            st_labels = pd.read_csv(
                os.path.join(dir_path, "subtype_labels"), index_col=0, skiprows=1
            )
        ...
        labels = pd.read_table(
            os.path.join(dir_path, "survival"), header=0, index_col=0, sep=r"\s+"
        )
    ...
    if aligned:
        ...
        for sample in labels.index:
            assert (
                len(sample.split(sample_delimiter_label)) == sample_len_label
            ), f"Sample {sample} does not have the expected format"
        ...
        for sample in intersection_omic:
            assert (
                len(sample.split(sample_delimiter_omic)) == sample_len_omic
            ), f"Sample {sample} does not have the expected format"

        match_length = min(sample_len_label, sample_len_omic)
        standardized_all_omic = {
            sample_delimiter_omic.join(
                sample.split(sample_delimiter_omic)[:match_length]
            ).lower(): sample
            for sample in intersection_omic
        }
        standardized_labels = {
            sample_delimiter_omic.join(
                sample.split(sample_delimiter_label)[:match_length]
            ).lower(): sample
            for sample in labels.index
        }

        intersection = set(standardized_all_omic.keys()).intersection(
            set(standardized_labels.keys())
        )
        ...
        assert len(intersection_all_omic) == len(
            intersection_all_labels
        ), f"Number of samples in omics {len(intersection_all_omic)} and labels {len(intersection_all_labels)} do not match"
        ...
        return {
            ...
            "labels": (
                labels.loc[intersection_all_labels]["Death"].values
                if not subtype_labels
                else labels.loc[intersection_all_labels].values
            ),
            # Leave these for compatibility with existing code
    ...
    for omic, data in zip(
        ["methylation", "mrna", "mirna"], [methylation_data, mrna_data, mirna_data]
    ):
        intersection_omic = [
            sample
            for sample in data.index
            if ".".join(sample.split(".")[:3]).lower() in labels.index.str.lower()
        ]
        intersection_labels = [
            ".".join(sample.split(".")[:3]).lower() for sample in intersection_omic
        ]

        assert len(intersection_omic) == len(
            intersection_labels
        ), f"Number of samples in omics {len(intersection_omic)} and labels {len(intersection_labels)} do not match"
...
def load_multiomics(dataset, **kwargs):
    if dataset in ALL_MULTIOMICS_DATASETS:
        dict_ds = load_multiomics_benchmark_ds(
            dataset, preprocessing=kwargs.get("preprocessing", "Original"), **kwargs
        )
    ...
    if dataset in ALL_MULTIOMICS_DATASETS_SHAMIR:
        ...
        return dict_ds, labels
    raise ValueError(
        f"Dataset {dataset} not recognized. Available datasets: {ALL_MULTIOMICS_DATASETS.union(ALL_MULTIOMICS_DATASETS_SHAMIR)}"
    )
